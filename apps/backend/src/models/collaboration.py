...
class WorkflowComment(Document):
    ...
    # Comment location
    node_id: Optional[str] = Field(None, description="Specific node (if applicable)")
    ...
    # Metadata
    created_at: datetime = Field(default_factory=datetime.utcnow, description="Thread creation")
...
class WorkflowChange(Document):
    ...
    user_name: str = Field(..., description="User display name")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="When changed")
